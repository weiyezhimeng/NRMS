...
def loss(history,label,batch,model_user,model_news,tokenizer,device):
	num=0
	...
	for i in range(batch):
		...
		#计算预测目标news的编码信息
		target_tokens_1_0 = tokenizer(prompt_1_and_0, return_tensors="pt", padding=True)['input_ids'].to(device)
		logits_1_0=model_news(target_tokens_1_0)
...
def test(file,model_user,model_news,tokenizer,device,start,end):
	result=[]
	...
	for i in tqdm(range(start,end)):
		with torch.no_grad():
			...
			logits_user=model_user(input_ids,attention_mask=attention_mask)[0]
			...
			logits=model_news(input_ids=a_for,attention_mask=b_for)[0]
